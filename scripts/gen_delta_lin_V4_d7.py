#!/usr/bin/env python3
"""
Generate the explicit polynomial form of the V_4-only perturbation at deg 7:
  (bch(x + V_4, ½a) − bch(x, ½a))_deg7  where x = ½a + b
                                              V_4 = bch_quartic_term(½a, b)

V_4 has degree 4 in {a, b}; at deg 7 the only contribution is:
  • Linear-in-V_4 from C_4 (deg 4 + (4-1) = 7).
(Quadratic-in-V_4 from C_k gives k + 2·(4-1) = k+6 ≥ 7, but with k ≥ 1 the
quadratic-in-V_4 from C_1 is impossible — C_1 has only 1 letter so no
substitution; quadratic-in-V_4 from C_k for k ≥ 2 gives k+6 ≥ 8.)

This is the simplest of the V_j-only deg-7 pieces (single operator term).
One ingredient of the future operator-form Phase B-septic identity
...
def main():
    ...
    bch_half_a_b = bch_series(half_a, b, 4)
    V4 = extract_degree(bch_half_a_b, 4)

    LCM_V4 = 1
    for c in V4.values():
        if c != 0:
            LCM_V4 = sp.lcm(LCM_V4, sp.denom(sp.nsimplify(c)))
    LCM_V4 = int(LCM_V4)
    sum_abs_V4 = sum(abs(int(sp.nsimplify(c * LCM_V4))) for c in V4.values() if c != 0)
    print(f"-- V_4 = bch_quartic_term(½a, b): {sum(1 for c in V4.values() if c != 0)} terms, "
          f"LCM = {LCM_V4}, Σ|num| = {sum_abs_V4}")

    # Compute (bch(x + V_4, ½a) - bch(x, ½a))_d7.
    # MAX = 7 enough: V_4 is deg 4, quadratic-in-V_4 is deg ≥ 4+(4-1)+(4-1)=10 ≥ 8, so safe.
    MAX = 7
    bch_x_V4 = bch_series(ncpoly_add(x, V4), half_a, MAX)
    bch_x = bch_series(x, half_a, MAX)
    diff = ncpoly_sub(bch_x_V4, bch_x)
    ...
    print()
    print("/-- **V_4-only perturbation at deg 7** (Phase B-septic foundation).")
    print()
    print("`(bch(x + V_4, ½a) − bch(x, ½a))_deg7`  where x = ½a + b, V_4 = bch_quartic_term(½a, b).")
    print()
    print("V_4 has deg 4 in {a, b}. At deg 7 the ONLY contribution is:")
    print("  • Linear-in-V_4 from C_4 (deg 4 + (4-1) = 7).")
    print("(Quadratic-in-V_4 is deg ≥ 8 for any non-trivial C_k.)")
    print()
    print(f"CAS-derived; denominator {LCM}, {len(items)} terms, Σ|num|/LCM ≈ {sum_abs/LCM:.4f}.")
    print()
    print("This is one of the simpler V_j-only deg-7 pieces (single linear operator).")
