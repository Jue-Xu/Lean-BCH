#!/usr/bin/env python3
"""
CAS verification: confirm that septic_d8_cross_V3_V4_poly equals the
ΔC_3_bil(V_3, V_4) operator form.

Cross(V_3, V_4)_d8 = mixed second-difference of bch_series at deg 8.
At deg 8, the only contribution is bilinear V_3·V_4 from C_3 (deg
3 + (3-1) + (4-1) = 8). Higher C_p gives total degree > 8.
...
d8 analog of verify_cross_V2_V4_operator_form.py (V_2 → V_3, V_4 → V_4).
"""
...
def main():
    ...
    triple1 = ncpoly_bracket(V3, bracket_V4_a)  # [V_3, [V_4, a']]
    triple2 = ncpoly_bracket(V4, bracket_V3_a)  # [V_4, [V_3, a']]

    operator_form = ncpoly_scale(ncpoly_add(triple1, triple2), one_twelfth)
    print("== Operator form (1/12)·([V_3, [V_4, a']] + [V_4, [V_3, a']]) ==")
    summarize(operator_form, "operator_form")

    cross_34 = cross(x, V3, V4, half_a, 8, 8)
    print()
    print("== Direct Cross(V_3, V_4)_d8 ==")
    summarize(cross_34, "cross_V3_V4_direct")

    diff = ncpoly_diff(operator_form, cross_34)
    nz = sum(1 for c in diff.values() if c != 0)
    print()
    if nz == 0:
        print("✓ VERIFIED: operator_form == Cross(V_3, V_4)_d8")
        print("  ΔC_3_bil(V_3, V_4) operator equals septic_d8_cross_V3_V4_poly exactly.")
    else:
