#!/usr/bin/env python3
"""
Generate the explicit polynomial form of the V_2·V_4 cross perturbation at deg 7:
  Cross(V_2, V_4) := (bch(x + V_2 + V_4, ½a) − bch(x + V_2, ½a)
                     − bch(x + V_4, ½a) + bch(x, ½a))_deg7

where x = ½a + b, V_2 = ½·[½a, b], V_4 = bch_quartic_term(½a, b).

This is the MIXED-V_2-and-V_4 part of the perturbation at deg 7: at deg 7,
the only contribution is:
  • Bilinear V_2·V_4 from C_3 (deg 3 + 1 + 3 = 7).
(Higher-order in V_2 or V_4 gives deg ≥ 8.)
...
def main():
    ...
    print()
    print("/-- **Cross V_2·V_4 perturbation at deg 7** (Phase B-septic foundation).")
    print()
    print("`Cross(V_2, V_4) := (bch(x + V_2 + V_4, ½a) − bch(x + V_2, ½a)")
    print("                    − bch(x + V_4, ½a) + bch(x, ½a))_deg7`")
    print()
    print("where x = ½a + b, V_2 = ½·[½a, b], V_4 = bch_quartic_term(½a, b).")
    print()
    print("At deg 7 only one contribution remains:")
    print("  • Bilinear V_2·V_4 from C_3 (deg 3 + 1 + 3 = 7).")
    print()
    ...
    print("-- Cross(V_2, V_4) = bch(x+V_2+V_4, ½a) - bch(x+V_2, ½a) - bch(x+V_4, ½a) + bch(x, ½a)")
