...
- **Stage 3.0 (complete)**: defined `BCH.symmetric_bch_septic_poly`
  ...
  (`c⁷` homogeneity, 1-line via `simp only`). The deg-7 analog of
  `symmetric_bch_quintic_poly`. (~162 lines in `BCH/SymmetricQuintic.lean`.)

  **Norm bound deferred**: `norm_symmetric_bch_septic_poly_le` is
  generated by `scripts/gen_septic_poly_norm_bound.py` (~800 lines via
  session 23 Finset.sum + uniform max bound pattern) but not yet inserted;
  Stage 2 needs it.
