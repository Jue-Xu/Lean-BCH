...
/-- **Linear-in-residual part of `sym_quintic_poly(α•V + δa, β•V + δb)`** —
the deg-(4,1) Lie content of the substituted quintic polynomial.

The `symmetric_bch_quintic_poly` is a Lie polynomial; by Witt's formula, the
free-Lie-algebra dimension at multi-degree (4, 1) in (V, δ) is 1, with
basis `ad_V^4(δ) = [V, [V, [V, [V, δ]]]]`. So the linear-in-(δa, δb) part
of the substituted polynomial equals
  `c_a • ad_V^4(δa) + c_b • ad_V^4(δb)`
for some scalars `c_a, c_b` polynomial in `(α, β)`.

CAS verification (`scripts/derive_quintic_linear_part.py`) gives:
  `c_a = -(β/5760) · (7α³ + 28α²·β + 32α·β² + 8β³)`
  `c_b = (α/5760) · (7α³ + 28α²·β + 32α·β² + 8β³)`

Closed form:
  `L = (P(α,β) / 5760) • (α • ad_V^4(δb) - β • ad_V^4(δa))`
  where `P(α, β) := 7α³ + 28α²·β + 32α·β² + 8β³`.

Used in the τ⁷ matching identity: at deg-3 strangBlock contributions
(δa = 4(pτ)³·E_3, δb = ((1-4p)τ)³·E_3), this gives the L-piece's τ⁷
content via `ad_V^4(E_3)`. Stepping-stone for `norm_septic_match_residual_le_axiom`
discharge. -/
noncomputable def sym_quintic_poly_linear_part_smul_V
    {𝕂 : Type*} [RCLike 𝕂] {𝔸 : Type*} [NormedRing 𝔸] [NormedAlgebra 𝕂 𝔸]
    (V : 𝔸) (α β : 𝕂) (δa δb : 𝔸) : 𝔸 :=
  ((5760 : 𝕂)⁻¹ * (7 * α ^ 3 + 28 * α ^ 2 * β + 32 * α * β ^ 2 + 8 * β ^ 3)) •
    (α • commBr V (commBr V (commBr V (commBr V δb))) -
     β • commBr V (commBr V (commBr V (commBr V δa))))
...
include 𝕂 in
omit [NormOneClass 𝔸] [CompleteSpace 𝔸] in
/-- **Septic-matching infrastructure (L-piece of E_5 at deg-3 E_3 inputs)**:
deg-3-input analog of `sym_cubic_poly_linear_part_at_smul_E3` but at one
degree higher (`sym_E_5` instead of `sym_E_3`). Computes the closed form
for the linear-in-residual part of `sym_E_5(α•V + γ·E_3, β•V + δ·E_3)`:

  `sym_quintic_poly_linear_part_smul_V V α β (γ•E_3) (δ•E_3) =
     ((5760)⁻¹ * P(α, β) * (α·δ − β·γ)) • [V, [V, [V, [V, E_3]]]]`

where `P(α, β) := 7α³ + 28α²·β + 32α·β² + 8β³`.

Derivation: by definition,
  `sym_quintic_poly_linear_part_smul_V V α β δa δb =
     ((5760)⁻¹ · P(α,β)) • (α · ad_V^4(δb) - β · ad_V^4(δa))`.
Substituting `δa = γ·E_3, δb = δ·E_3` and pulling `γ, δ` outside the
4-fold V-commutators (via `commBr_smul_right_eq`) gives
`(α·δ − β·γ) · ad_V^4(E_3)`. The combined scalar factor collapses by
`smul_smul + sub_smul`.

Used in the τ⁷ matching identity for the L-piece of `sym_E_5(4X, Y)` at
deg-3 strangBlock contributions. Stepping-stone for
`norm_septic_match_residual_le_axiom` discharge. -/
theorem sym_quintic_poly_linear_part_at_smul_E3 (A B V : 𝔸) (α β γ δ : 𝕂) :
    sym_quintic_poly_linear_part_smul_V V α β
        (γ • symmetric_bch_cubic_poly 𝕂 A B)
        (δ • symmetric_bch_cubic_poly 𝕂 A B) =
      ((5760 : 𝕂)⁻¹ * (7 * α ^ 3 + 28 * α ^ 2 * β + 32 * α * β ^ 2 + 8 * β ^ 3) *
        (α * δ - β * γ)) •
        commBr V (commBr V (commBr V
          (commBr V (symmetric_bch_cubic_poly 𝕂 A B)))) := by
  unfold sym_quintic_poly_linear_part_smul_V
  -- δa = γ • E_3, δb = δ • E_3. Push γ, δ through the 4-fold V-commutators
  -- (one commBr_smul_right_eq per nesting level), then combine into a single
  -- (α·δ - β·γ) • ad_V^4(E_3) factor via sub_smul.
  rw [show (α • commBr V (commBr V (commBr V
              (commBr V (δ • symmetric_bch_cubic_poly 𝕂 A B)))) -
            β • commBr V (commBr V (commBr V
              (commBr V (γ • symmetric_bch_cubic_poly 𝕂 A B))))) =
        (α * δ - β * γ) •
          commBr V (commBr V (commBr V
            (commBr V (symmetric_bch_cubic_poly 𝕂 A B)))) from by
    simp only [commBr_smul_right_eq (𝕂 := 𝕂), smul_smul, ← sub_smul]]
  rw [smul_smul]

include 𝕂 in
omit [NormOneClass 𝔸] [CompleteSpace 𝔸] in
/-- **Septic-matching scalar instantiation (L-piece of E_5 at deg-3 E_3 strangBlock)**:
substituting `α = 4pτ, β = (1-4p)τ, γ = 4(pτ)³, δ = ((1-4p)τ)³` (the τ⁵-leading
parts of B1.d's per-block residuals at deg-3 contributions), the L-piece
collapses to a closed form with τ⁷ factored out:

  `L = (-(1/180) * p * (1-4p) * (1-2p) * (1-5p) * (1-3p) * (12p² − 6p − 1) * τ⁷) •
         [V, [V, [V, [V, E_3]]]]`.

Derivation (CAS-verified in `scripts/derive_quintic_linear_part.py`):
* `P(α, β) = 7α³ + 28α²·β + 32α·β² + 8β³`. At `α = 4pτ, β = (1-4p)τ`:
  `P = -8(1-2p)(12p²-6p-1)·τ³` (factor via Horner reduction in `r = 1-4p`).
* `α·δ − β·γ = 4p(1-4p)·((1-4p)² − p²)·τ⁴ = 4p(1-4p)(1-5p)(1-3p)·τ⁴`.
* Product `(1/5760)·P·(α·δ-β·γ) = -(1/180)·p(1-4p)(1-2p)(1-5p)(1-3p)·(12p²−6p−1)·τ⁷`.

The 4 linear factors `(1-2p)(1-3p)(1-4p)(1-5p)` vanish at the natural Suzuki
roots `p ∈ {1/2, 1/3, 1/4, 1/5}`. The quadratic `12p² − 6p − 1` has irrational
roots `(3 ± √21)/12`.

This is the τ⁷ Childs-basis content of the L-piece of `sym_E_5(4X, Y)` when
`δa, δb` come from the deg-3 part of the per-block residual. -/
theorem sym_quintic_poly_linear_part_at_strangBlock_E3 (A B : 𝔸) (p τ : 𝕂) :
    sym_quintic_poly_linear_part_smul_V (A + B) (4 * p * τ) ((1 - 4 * p) * τ)
        ((4 * (p * τ) ^ 3) • symmetric_bch_cubic_poly 𝕂 A B)
        (((1 - 4 * p) * τ) ^ 3 • symmetric_bch_cubic_poly 𝕂 A B) =
      (-(1 / 180 : 𝕂) * p * (1 - 4 * p) * (1 - 2 * p) *
        (1 - 5 * p) * (1 - 3 * p) * (12 * p ^ 2 - 6 * p - 1) * τ ^ 7) •
        commBr (A + B) (commBr (A + B) (commBr (A + B)
          (commBr (A + B) (symmetric_bch_cubic_poly 𝕂 A B)))) := by
  rw [sym_quintic_poly_linear_part_at_smul_E3 (𝕂 := 𝕂) A B (A + B)
        (4 * p * τ) ((1 - 4 * p) * τ) (4 * (p * τ) ^ 3) (((1 - 4 * p) * τ) ^ 3)]
  congr 1
  ring
